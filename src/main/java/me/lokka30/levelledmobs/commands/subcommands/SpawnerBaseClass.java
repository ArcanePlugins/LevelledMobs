...
public class SpawnerBaseClass extends MessagesBase {
    ...
    @Nullable
    String getArgValue(final String key, final String @NotNull [] args,
        final boolean mustBeNumber) {
        int keyFlag = -1;
        int nameStartFlag = -1;
        int nameEndFlag = -1;
    ...
    static void setMetaItems(final @Nullable ItemMeta meta, final @NotNull CustomSpawnerInfo info,
        ...
        if (ver.getIsRunningPaper()
            ...
            SpigotUtils.updateItemDisplayName(meta,
                info.customName == null ? defaultName : info.customName);
        }
        ...
        try {
            ...
            for (final Field f : info.getClass().getDeclaredFields()) {
                ...
                if ("-1".equals(f.get(info).toString()) && (name.equals("minLevel") || name.equals(
                    "maxLevel"))) {
                    continue;
                }
        ...
        if (!info.noLore && info.lore == null && info.customLore == null) {
            ...
            meta.getPersistentDataContainer()
                .set(info.main.namespacedKeys.keySpawnerLore, PersistentDataType.STRING,
                    sbLore.toString());
            ...
            meta.getPersistentDataContainer()
                .set(info.main.namespacedKeys.keySpawnerLore, PersistentDataType.STRING, useLore);
        }
    }
