...
public class WorldGuardIntegration {
    ...
    @NotNull
    public static List<String> getWorldGuardRegionOwnersForLocation(@NotNull final LivingEntityInterface lmInterface) {
        final List<String> wg_Owners = new LinkedList<>();

        if (lmInterface.getWorld() == null) return wg_Owners;

        final Set<ProtectedRegion> regions = getRegionSet(lmInterface);
        if (regions == null) return wg_Owners;
    ...
    @Nullable
    private static Set<ProtectedRegion> getRegionSet(@NotNull final LivingEntityInterface lmInterface){
