...
public class LMobSpawn implements Listener {
    ...
    public boolean isAllowed(EntityType entityType) {
        List<String> blacklistedTypes = instance.settings.getStringList("blacklisted-types");
        for (String blacklistedType : blacklistedTypes) {
            if (entityType.toString().equalsIgnoreCase(blacklistedType)) {
                return false;
            }
        }
        return true;
